...
# Решил ещё одним способом данную задачу, опираясь не на алгоритм, а
# на свойства словарей, все локальные тесты проходит и даёт верный
# ответ, однако на платформе в компиляторе Make выдаёт какую-то чушь.
# Хотел спросить у Вас адекватное и легитимное альтернативное решение
# у меня получилось или нет, спасибо заранее за ответ, ниже представлен код
...
#     x = {i: 0 for i in input().split()}         # СЧИТЫВАЕМ МАССИВ ДАННЫХ И ИСПОЛЬЗУЯ DICT COMPREHENSION
